    ...
    <div class="d-flex align-content-center flex-wrap gap-4">
        ...
        <?php if ($usulan->status == 4): ?>
            <a href="<?= site_url('supervisor/usulan/pendirianptkis/penilaianasesor/' . encrypt($usulan->id)) ?>" type="button"
                ...
                onclick="return confirm('Apakah Anda yakin ingin mengirim ke Penilai?')">Kirim ke Penilai</a>
        <?php endif; ?>
        <?php if ($usulan->status == 5): ?>
            <a href="<?= site_url('supervisor/usulan/pendirianptkis/prosesrkma/' . encrypt($usulan->id)) ?>" type="button"
                class="btn btn-success waves-effect waves-light"
                onclick="return confirm('Apakah proses penilaian selesai?')">Proses RKMA</a>
        <?php endif; ?>
    </div>
</div>
    ...
    <ul class="nav nav-pills flex-column flex-md-row mb-6 row-gap-2">
        <li class="nav-item">
            <a class="nav-link waves-effect waves-light"
                href="<?= site_url('supervisor/usulan/pendirianptkis/detail/' . encrypt($usulan->id)) ?>"><i
    ...
    <div class="col-sm-12">
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kecukupan">Tambah
                    Asesor</button>
            </div>
            <table class="table table-bordered">
                ...
                <?php foreach ($asesorkecukupan as $ak): ?>
                    <tr>
                        ...
                        <td><?= ($ak->file_hasil) ? '<a href="' . base_url('uploads/nilai/' . $ak->file_hasil) . '" target="_blank">Unduh</a>' : 'Belum Mengunggah'; ?>
                        </td>
                        <td><?= $ak->skor ?></td>
                        <td>
                            <?php
                            if ($ak->status == 2) {
                                ...
                                <a href="<?= site_url('supervisor/usulan/pendirianptkis/detail/penilaian/review/' . $ak->id) ?>"
                                    ...
                                    onclick="return confirm('Nilai akan dikembalikan ke Asesor untuk ditinjau ulang?')">Kembalikan</a>
                            <?php } else { ?>
                                <a href="<?= site_url('supervisor/usulan/pendirianptkis/asesor/delete/' . $ak->id) ?>"
                                    class="btn btn-sm btn-danger"
                                    onclick="return confirm('Asesor Kecukupan akan dihapus?')">Hapus</a>
                            <?php } ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
        ...
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#lapangan">Tambah
                    Asesor</button>
            </div>
            <table class="table table-bordered">
                <tr>
                ...
                <?php foreach ($asesorlapangan as $al): ?>
                    <tr>
                        ...
                        <td><?= $al->file_hasil ?></td>
                        <td><?= $al->skor ?></td>
                        <td><a href="<?= site_url('supervisor/usulan/pendirianptkis/asesor/delete/' . $al->id) ?>"
                                class="btn btn-sm btn-danger"
                                onclick="return confirm('Asesor Lapangan akan dihapus?')">Hapus</a></td>
                    </tr>
                <?php endforeach; ?>
            </table>
        </div>
        ...
        <div class="modal-content text-center">
            ...
            <div class="modal-body p-0">
                <div class="onboarding-content mb-0">
                    ...
                    <form action="<?= site_url('supervisor/usulan/pendirianptkis/asesor/add') ?>" method="post"
                        id="usulform">
                        <div class="row mb-6">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content text-center">
            ...
            <div class="modal-footer border-0">
                ...
                <button type="button" class="btn btn-primary"
                    onclick="document.getElementById('usulform2').submit();">Simpan</button>
            </div>
        </div>
    </div>
</div>
