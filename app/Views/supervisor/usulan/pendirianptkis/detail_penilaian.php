...
<div class="nav-align-top mb-5">
    <ul class="nav nav-pills flex-column flex-md-row row-gap-2" style="background: white; border-radius: 12px; padding: 6px;">
        ...
        <li class="nav-item">
            <a href="<?= site_url('supervisor/usulan/pendirianptkis/detail/penilaian/' . encrypt($usulan->id)) ?>" class="nav-link active waves-effect waves-light" style="border-radius: 8px;">
    ...
    <div class="col-sm-12">
        <div class="card mb-4" style="border: none; border-radius: 14px;">
            <div class="card-header py-3 px-4 d-flex justify-content-between align-items-center" style="border-bottom: 1px solid #f0f2f5;">
                ...
                <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#kecukupan" style="border-radius: 8px;">
                ...
                <table class="table mb-0">
                    <thead>
                        <tr style="background: #f8f9fb;">
                            <th style="padding: 12px 20px; font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.08em; font-weight: 700; color: #8c97a7; border: none;">Asesor</th>
                            ...
                            <th style="padding: 12px 20px; font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.08em; font-weight: 700; color: #8c97a7; border: none;">Lampiran</th>
                            <th style="padding: 12px 20px; font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.08em; font-weight: 700; color: #8c97a7; border: none;">Nilai</th>
                    ...
                    <tbody>
                        <?php if (empty($asesorkecukupan)): ?>
                            <tr>
                                <td colspan="5" class="text-center py-4 text-muted" style="border: none;">Belum ada asesor kecukupan yang ditugaskan.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($asesorkecukupan as $ak): ?>
                                <tr class="doc-row">
                                    <td style="padding: 12px 20px; border-bottom: 1px solid #f0f2f5; vertical-align: middle; font-weight: 600; color: #566a7f;"><?= $ak->full_name ?></td>
                                    <td style="padding: 12px 20px; border-bottom: 1px solid #f0f2f5; vertical-align: middle; color: #8c97a7; font-size: 0.85rem;"><?= $ak->mulai_tanggal . ' - ' . $ak->sampai_tanggal ?></td>
        ...
        <div class="card mb-4" style="border: none; border-radius: 14px;">
            <div class="card-header py-3 px-4 d-flex justify-content-between align-items-center" style="border-bottom: 1px solid #f0f2f5;">
                ...
                <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#lapangan" style="border-radius: 8px;">
                    <i class="ti tabler-plus me-1"></i>Tambah Asesor
                </button>
            </div>
            <div class="table-responsive">
                <table class="table mb-0">
                    ...
                    <tbody>
                        <?php if (empty($asesorlapangan)): ?>
                            <tr>
                                <td colspan="5" class="text-center py-4 text-muted" style="border: none;">Belum ada asesor lapangan yang ditugaskan.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($asesorlapangan as $al): ?>
                                <tr class="doc-row">
                                    <td style="padding: 12px 20px; border-bottom: 1px solid #f0f2f5; vertical-align: middle; font-weight: 600; color: #566a7f;"><?= $al->full_name ?></td>
                                    <td style="padding: 12px 20px; border-bottom: 1px solid #f0f2f5; vertical-align: middle; color: #8c97a7; font-size: 0.85rem;"><?= $al->mulai_tanggal . ' - ' . $al->sampai_tanggal ?></td>
                                    <td style="padding: 12px 20px; border-bottom: 1px solid #f0f2f5; vertical-align: middle;">
...
<div class="modal fade" id="kecukupan" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content" style="border: none; border-radius: 16px; overflow: hidden;">
            <div class="modal-header py-3 px-4" style="background: linear-gradient(135deg, #03c3ec, #71d8f7); border: none;">
                <h5 class="modal-title text-white fw-semibold"><i class="ti tabler-user-plus me-2"></i>Penugasan Asesmen Kecukupan</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="<?= site_url('supervisor/usulan/pendirianptkis/asesor/add') ?>" method="post" id="usulform">
                <div class="modal-body p-4">
                    <div class="mb-4">
                        <label class="form-label fw-semibold" style="font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.06em; color: #566a7f;">Asesor</label>
                    ...
                    <div class="row mb-4 g-3">
                        <div class="col-sm-6">
                            <label class="form-label fw-semibold" style="font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.06em; color: #566a7f;">Tgl Mulai</label>
                            <input type="date" class="form-control" id="mulai_tanggal" name="mulai_tanggal" style="border-radius: 10px;" required>
                        </div>
                        <div class="col-sm-6">
                            <label class="form-label fw-semibold" style="font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.06em; color: #566a7f;">Tgl Selesai</label>
                            <input type="date" class="form-control" id="sampai_tanggal" name="sampai_tanggal" style="border-radius: 10px;" required>
                        </div>
                    </div>
                    <div class="mb-2">
                        <label class="form-label fw-semibold" style="font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.06em; color: #566a7f;">Keterangan</label>
                        <textarea name="keterangan" class="form-control" id="keterangan" rows="3" style="border-radius: 10px;"></textarea>
                    </div>
                </div>
                <div class="modal-footer" style="border-top: 1px solid #f0f2f5;">
                    <button type="button" class="btn btn-label-secondary" data-bs-dismiss="modal" style="border-radius: 8px;">Tutup</button>
                    <button type="button" class="btn btn-primary" onclick="document.getElementById('usulform').submit();" style="border-radius: 8px;">Simpan</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="lapangan" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content" style="border: none; border-radius: 16px; overflow: hidden;">
            <div class="modal-header py-3 px-4" style="background: linear-gradient(135deg, #71dd37, #a0e87a); border: none;">
                <h5 class="modal-title text-white fw-semibold"><i class="ti tabler-user-plus me-2"></i>Penugasan Asesmen Lapangan</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="<?= site_url('supervisor/usulan/pendirianptkis/asesor/add') ?>" method="post" id="usulform2">
                ...
                <div class="modal-footer" style="border-top: 1px solid #f0f2f5;">
                    <button type="button" class="btn btn-label-secondary" data-bs-dismiss="modal" style="border-radius: 8px;">Tutup</button>
                    <button type="button" class="btn btn-primary" onclick="document.getElementById('usulform2').submit();" style="border-radius: 8px;">Simpan</button>
                </div>
            </form>
        </div>
    </div>
