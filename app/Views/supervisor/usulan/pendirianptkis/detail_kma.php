...
<div class="nav-align-top mb-5">
    <ul class="nav nav-pills flex-column flex-md-row row-gap-2" style="background: white; border-radius: 12px; padding: 6px;">
        ...
        <li class="nav-item">
            <a href="<?= site_url('supervisor/usulan/pendirianptkis/detail/kma/' . encrypt($usulan->id)) ?>" class="nav-link active waves-effect waves-light" style="border-radius: 8px;">
        ...
        <div class="card" style="border: none; border-radius: 14px;">
            ...
            <div class="card-body p-4">
                <form action="<?= site_url('supervisor/usulan/detail/kma/save/'.encrypt($usulan->id)) ?>" method="post" enctype="multipart/form-data">
                    <div class="row mb-4">
                        <label class="col-sm-3 col-form-label fw-medium" for="no_kma" style="color: #566a7f;">No Keputusan</label>
                        <div class="col-sm-9">
                            <input type="text" id="no_kma" name="no_kma" class="form-control" value="<?= $usulan->no_kma ?>" style="border-radius: 10px;" />
                        </div>
                    </div>
                    <div class="row mb-4">
                        <label class="col-sm-3 col-form-label fw-medium" for="tgl_kma" style="color: #566a7f;">Tanggal Keputusan</label>
                        <div class="col-sm-9">
                            <input type="date" id="tgl_kma" name="tgl_kma" class="form-control" value="<?= $usulan->tgl_kma ?>" style="border-radius: 10px;" />
                        </div>
                    </div>
                    <div class="row mb-5">
                        <label class="col-sm-3 col-form-label fw-medium" for="lampiran" style="color: #566a7f;">File Keputusan</label>
                        <div class="col-sm-9">
                            <div class="input-group">
                                <input type="file" class="form-control" id="lampiran" name="lampiran" aria-describedby="groupLampiran" aria-label="Upload" accept=".pdf" style="border-radius: 10px 0 0 10px;" />
                                <?php if($usulan->file_kma): ?>
                                    <a class="btn btn-outline-primary" type="button" id="groupLampiran" href="<?= base_url('uploads/kma/'.$usulan->file_kma) ?>" target="_blank" style="border-radius: 0 10px 10px 0;">
                                        <i class="ti tabler-eye me-1"></i>Lihat
                                    </a>
                                <?php else: ?>
                                    <span class="input-group-text bg-label-secondary" id="groupLampiran" style="border-radius: 0 10px 10px 0;">Belum ada file</span>
                                <?php endif; ?>
                            </div>
                            <small class="text-muted mt-2 d-block"><i class="ti tabler-info-circle me-1" style="font-size: 0.9rem;"></i>Hanya format PDF yang diperbolehkan.</small>
                        </div>
                    </div>
