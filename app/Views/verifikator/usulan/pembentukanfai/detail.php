...
$(document).ready(function() {
    $('.formcheck').change(function(event) {
        if(this.checked) {
            $.get('<?= site_url('verifikator/usulan/pembentukanfai/validasidokumen');?>/'+this.id+'/1/0', function() {
            alert('Berkas divalidasi');
            });
        }else{
            $.get('<?= site_url('verifikator/usulan/pembentukanfai/validasidokumen');?>/'+this.id+'/0/'+$('#keterangan_dokumen'+this.id).val(), function() {
    ...
    $('.keterangancheck').change(function(event) {
        if(this.value) {
            $.get('<?= site_url('verifikator/usulan/pembentukanfai/validasidokumen');?>/'+this.dataset.dok+'/0/'+this.value, function() {
...
function declined() {
  Swal.fire({
    ...
    preConfirm: (data) => {
      return fetch('<?= site_url('verifikator/usulan/pembentukanfai/decline/'.encrypt($usulan->id)) ?>', {
        method: "POST",
  ...
  function accept() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        window.location.href = '<?= site_url('verifikator/usulan/pembentukanfai/accept/'.encrypt($usulan->id)) ?>';
      }
    });
  }
